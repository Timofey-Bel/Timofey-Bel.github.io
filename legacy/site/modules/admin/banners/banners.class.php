...
class banners extends aModule
{
    /**
     * API модуль управления баннерами
     */
    function execute($arr)
    {
        ...
        if (!ACL::check('banners', $requiredPermission)) {
            header('Content-Type: application/json');
            echo json_encode([
                'success' => false,
    ...
    private function create_banner()
    {
        ...
        $query = "INSERT INTO banners (url, name, title, sort) 
    ...
    private function delete_banner()
    {
        ...
        $banner = row("SELECT url FROM banners WHERE id = $id");
    ...
    private function upload_banner()
    {
        ...
        $file = $_FILES['file'];
        $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/assets/medx/img/banners/';
        ...
        if (move_uploaded_file($file['tmp_name'], $filepath)) {
            $url = '/assets/medx/img/banners/' . $filename;
    ...
    private function sort_banners()
    {
        ...
        foreach ($data['banners'] as $banner) {
            ...
            q("UPDATE banners SET sort = $sort WHERE id = $id");
        }
     ...
     * Использует существующую логику updateSort
     */
    private function update_sort()
    {
        ...
        foreach ($order as $id) {
            $id = intval($id);
            if ($id > 0) {
                q("UPDATE banners SET sort = $i WHERE id = $id");
                $i++;
            }
        }
